...
public enum ElementType { Ice,Fire,None}
...
public abstract class Knight : MonoBehaviour {
    ...
    /// 攻击元素属性
    /// </summary>
    protected ElementType e_Type;

    public ElementType E_Type
    {
        get; protected set;
    }
    ...
    public void Attack(Enemy enemy)
    {
        ...
        if ((attacking_Enemy == null || !attacking_Enemy.IsActive) && enemies_InTrigger.Count > 0)
        {
            attacking_Enemy = enemies_InTrigger[enemies_InTrigger.Count - 1];
        }
        else if (attacking_Enemy != null && attacking_Enemy.IsActive)
        {
            if (canAttack)
            {
                Knight_Attack();
        ...
        }else if(attacking_Enemy == null && enemies_InTrigger.Count <= 0)
        {
            Idle();
        }
    }
